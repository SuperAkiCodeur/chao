...
export default async function HomePage() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      {/* ── Top bar 60px ── */}
      <div style={{ height: 60, flexShrink: 0, display: "flex", alignItems: "center", padding: "0 20px", gap: 10, borderBottom: BD }}>
        {/* Search */}
        <div style={{ position: "relative", flex: 1, maxWidth: 340 }}>
          ...
          <input readOnly placeholder="Rechercher…" style={{ width: "100%", height: 34, background: "rgba(255,255,255,0.05)", border: BD, borderRadius: 8, paddingLeft: 30, paddingRight: 12, fontSize: 13, color: "rgba(255,255,255,0.40)", outline: "none" }} />
        </div>
        <div style={{ flex: 1 }} />
        {/* Pills */}
        {[
          ...
          <div key={label} style={{ display: "flex", alignItems: "center", gap: 6, background: "#242424", border: BD, borderRadius: 99, padding: "5px 12px" }}>
        ...
        {/* Avatar */}
        <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#2A2A2A", border: BD, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 12, color: "rgba(255,255,255,0.65)", flexShrink: 0 }}>C</div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "20px", display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {/* ── Hero ── */}
        <div style={{ borderRadius: 12, background: "#1F1F1F", border: BD, padding: "24px", display: "flex", alignItems: "stretch", gap: 14, flexShrink: 0 }}>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "space-between", minWidth: 0 }}>
            <div>
              <p style={{ fontSize: 12, color: "rgba(255,255,255,0.35)", marginBottom: 8 }}>Bienvenue 👋</p>
              <h1 style={{ fontSize: "clamp(22px,2.2vw,32px)", fontWeight: 800, color: "#fff", lineHeight: 1.2, letterSpacing: "-0.03em", marginBottom: 20 }}>
                {activeCinema > 0
                  ? <>{activeCinema} séance{activeCinema > 1 ? "s" : ""}<br />active{activeCinema > 1 ? "s" : ""} en ce moment</>
                  : <>Bot Chao<br />est en ligne</>}
              </h1>
            </div>
            <a href="/cinema" style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "#fff", color: "#000", fontWeight: 700, fontSize: 13, padding: "9px 16px", borderRadius: 8, textDecoration: "none", alignSelf: "flex-start" }}>
              Voir les séances ↗
            </a>
          </div>
          {/* Mini-cards */}
          <div style={{ display: "flex", gap: 8, alignItems: "stretch", flexShrink: 0 }}>
            {[
              ...
              { n: "03", title: "Steam",    sub: "Catalogue",                                              bg: "#161616", fg: "#fff", muted: "rgba(255,255,255,0.28)" },
            ].map(({ n, title, sub, bg, fg, muted }) => (
              <div key={n} style={{ width: 110, borderRadius: 10, padding: "14px", background: bg, border: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: "column", justifyContent: "space-between", minHeight: 140 }}>
                <span style={{ fontSize: 10, fontWeight: 700, color: muted, letterSpacing: "0.05em" }}>{n}</span>
                <div>
                  <p style={{ fontSize: 16, fontWeight: 800, color: fg, letterSpacing: "-0.02em", lineHeight: 1.2 }}>{title}</p>
                  <p style={{ fontSize: 11, color: muted, marginTop: 3 }}>{sub}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 260px", gap: 10, flex: 1, minHeight: 0 }}>
          ...
          <div style={{ background: "#202020", borderRadius: 12, border: BD, overflow: "hidden", display: "flex", flexDirection: "column", minHeight: 0 }}>
            ...
            <div style={{ flex: 1, overflowY: "auto" }}>
              {recentCinema.length === 0 ? (
                <p style={{ padding: "20px", fontSize: 13, color: "rgba(255,255,255,0.20)" }}>Aucune séance pour l'instant.</p>
              ) : recentCinema.map((party, i) => (
                <div key={party.messageId} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 20px", borderBottom: i < recentCinema.length - 1 ? BD : undefined }}>
                  <div style={{ width: 32, height: 32, borderRadius: "50%", flexShrink: 0, background: "rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13 }}>
                    {party.status === "active" ? "▶" : "✓"}
                  </div>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 600, padding: "3px 9px", borderRadius: 99, flexShrink: 0, background: "rgba(255,255,255,0.07)", color: party.status === "active" ? "#fff" : "rgba(255,255,255,0.25)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, minHeight: 0 }}>
            ...
            {/* Stats */}
            <div style={{ background: "#202020", borderRadius: 12, border: BD, padding: "14px 20px" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {[
                  ...
                  <div key={label} style={{ background: "#282828", borderRadius: 9, padding: "10px 14px" }}>
            ...
            <div style={{ background: "#202020", borderRadius: 12, border: BD, padding: "14px 20px" }}>
              ...
              {[
                ...
                <a key={href} href={href} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 0", borderBottom: i < 2 ? BD : undefined, textDecoration: "none" }}>
