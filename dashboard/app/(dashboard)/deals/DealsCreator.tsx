  ...
  return (
    <div style={{ background: "#202020", borderRadius: 12, border: BD }}>
      <button type="button" onClick={() => setOpen((v) => !v)}
        ...
        <div style={{ width: 26, height: 26, borderRadius: 7, background: "rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        ...
        <CaretDown size={12} style={{ color: "rgba(255,255,255,0.22)", transform: open ? "rotate(180deg)" : undefined, transition: "transform 0.15s ease" }} />
      </button>
      ...
      {open && (
        <div style={{ borderTop: BD, padding: "16px 20px", display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ display: "grid", gridTemplateColumns: "80px 1fr", gap: 12, alignItems: "center" }}>
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ex : Wishlist été, Jeux co-op…" maxLength={50}
              style={{ height: 34, background: "rgba(255,255,255,0.05)", border: BDI, borderRadius: 8, padding: "0 12px", fontSize: 13, color: "#fff" }} />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "80px 1fr", gap: 12, alignItems: "center" }}>
            <label style={{ fontSize: 13, color: "rgba(255,255,255,0.45)", fontWeight: 500 }}>Salon</label>
            <ChannelSelect value={channelId} onChange={setChannelId} channels={available} placeholder="Salon où /deals sera utilisé…" />
          </div>
